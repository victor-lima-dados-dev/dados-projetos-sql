...
select*
from sales.products
where price = (select max(price) from sales.products) -- SUBQUERYS SÓ PODEM RETORNAR UM VALOR CASO CONTRARIO NÃO FUNCIONA.

-- (Exemplo 2) Subquery com WITH
-- Calcule a idade média dos clientes por status profissional

with alguma_tabela as (select professional_status,
