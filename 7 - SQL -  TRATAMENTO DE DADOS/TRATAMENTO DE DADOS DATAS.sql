...
select (current_date + interval '10 mouths') :: date
...
select
	date_trunc('month', visit_page_date)::date as visit_page_mouth,count(*)
from sales.funnel
group by visit_page_mouth
order by visit_page_mouth desc
...
select	
	extract('dow' from visit_page_date) as dia_da_semana, count(*)
from sales.funnel
